...
export function signApprovalPayload(payload: ApprovalPayload, secret = getEnv("APPROVAL_HMAC_SECRET")): string {
  if (!secret) throw new PatchPilotError("approval_secret_missing", "Set APPROVAL_HMAC_SECRET to sign approval callbacks.");
...
export function verifyApprovalToken(token: string, secret = getEnv("APPROVAL_HMAC_SECRET"), nowSeconds = Math.floor(Date.now() / 1000)): ApprovalPayload {
  if (!secret) throw new PatchPilotError("approval_secret_missing", "Set APPROVAL_HMAC_SECRET to verify approval callbacks.");
  const [encoded, sig] = token.split(".");
  if (!encoded || !sig) throw new PatchPilotError("approval_signature_invalid", "Approval callback signature is malformed.");
  const expected = crypto.createHmac("sha256", secret).update(encoded).digest("base64url");
  if (!crypto.timingSafeEqual(Buffer.from(sig), Buffer.from(expected))) {
    throw new PatchPilotError("approval_signature_invalid", "Approval callback signature is invalid.");
  }
  const payload = JSON.parse(Buffer.from(encoded, "base64url").toString("utf8")) as ApprovalPayload;
  if (payload.exp < nowSeconds) throw new PatchPilotError("approval_token_expired", "Approval callback token has expired.");
  return payload;
}
