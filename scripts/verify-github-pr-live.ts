...
async function main() {
  ...
  const wantPkg = process.env.PATCHPILOT_VERIFY_PACKAGE;
  ...
  const agent = (process.env.PATCHPILOT_VERIFY_AGENT as "deterministic-npm" | "ollama" | "codex" | undefined) ?? "deterministic-npm";
  ...
  if (job.status !== "push_pending") {
    throw new PatchPilotError("github_push_gate_not_reached", "Remediation did not reach the push gate.", { status: job.status, errorCode: job.errorCode, errorMessage: job.errorMessage });
  }
  ...
  if (process.env.PATCHPILOT_VERIFY_GATE_ONLY === "true") {
  ...
  // PR stays open for the live Telegram merge demo. Set PATCHPILOT_VERIFY_MERGE=true.
  // Or exercise the REVERT path with PATCHPILOT_VERIFY_REJECT=true (close PR + delete branch).
  let merged = false;
  let reverted = false;
  if (process.env.PATCHPILOT_VERIFY_REJECT === "true" && pushed.status === "pr_open") {
    const rejected = await service.rejectMerge(job.id, "verify:github-pr-live");
    reverted = rejected.status === "rejected";
  } else if (process.env.PATCHPILOT_VERIFY_MERGE === "true" && pushed.status === "pr_open") {
  ...
  if (!pr?.url) {
    throw new PatchPilotError("github_pr_live_failed", "Live GitHub PR verification did not create a PR.", { status: pushed.status, errorCode: pushed.errorCode, errorMessage: pushed.errorMessage });
  }
