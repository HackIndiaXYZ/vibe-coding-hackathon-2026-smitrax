...
> Naming note: the implemented LLM provider keys are PatchPilot-prefixed and
...
|---|---|---|---|---|
| `PATCHPILOT_DATA_FILE` | 🟢 | `.patchpilot/patchpilot.db.json` | Local JSON state store | Uses default path |
| `PATCHPILOT_LOG_DIR` | 🟢 | `.patchpilot/logs` | Validation/command logs | Uses default path |
| `PATCHPILOT_WORKSPACE_DIR` | 🟢 | `.patchpilot/workspaces` | Disposable remediation workspaces | Uses default path |
...
| `PATCHPILOT_ANTHROPIC_BASE_URL` | 🟢 | `https://api.anthropic.com` | Anthropic endpoint | default |
| `PATCHPILOT_ANTHROPIC_VERSION` | 🟢 | `2023-06-01` | Anthropic API version header | default |
| `PATCHPILOT_GROK_API_KEY` | 🔒 | (falls back to `PATCHPILOT_LLM_API_KEY`) | Grok/xAI provider | `not_configured` |
| `PATCHPILOT_GROK_BASE_URL` | 🟢 | `https://api.x.ai/v1` | Grok endpoint | default |

## Provider failover ladder
| Key | Secret | Default | Feature |
|---|---|---|---|
| `PATCHPILOT_PROVIDER_FAILOVER_MODE` | 🟢 | `ask` | `ask`/`automatic`/`disabled` |
| `PATCHPILOT_PROVIDER_CHAIN` | 🟢 | `codex,openrouter,anthropic,grok,openai-compatible,ollama,deterministic` | Ordered chain |
| `PATCHPILOT_ALLOW_CLOUD_MODEL_FAILOVER` | 🟢 | `true` | Cloud→cloud auto-failover |
| `PATCHPILOT_ALLOW_LOCAL_MODEL_FAILOVER` | 🟢 | `false` | Use local model without consent |
| `PATCHPILOT_REQUIRE_CONSENT_FOR_LOWER_TRUST_PROVIDER` | 🟢 | `true` | Consent before local/deterministic in ask mode |
| `PATCHPILOT_FAST_FAILOVER` | 🟢 | `true` | Fast-failover behaviour |
| `PATCHPILOT_PROVIDER_CHAIN_MAX_ATTEMPTS` | 🟢 | `3` | Max provider attempts |
| `PATCHPILOT_PROVIDER_READINESS_TIMEOUT_MS` | 🟢 | `3000` | Readiness probe timeout |
| `PATCHPILOT_PROVIDER_ATTEMPT_TIMEOUT_MS` | 🟢 | `30000` | Per-attempt timeout |
| `PATCHPILOT_PROVIDER_READINESS_CACHE_TTL_MS` | 🟢 | `600000` | Readiness cache TTL |
...
`GITLEAKS`, `SEMGREP`, `TRIVY`, `SYFT`. Plus `PATCHPILOT_SCANNER_TIMEOUT_MS`
(default `120000`), `PATCHPILOT_MALICIOUS_PACKAGES_DIR` (OpenSSF data dir →
enables "malicious" labels), `PATCHPILOT_LICENSE_POLICY_PATH` (allowed/review/
...
`OPENCLAW_ENABLED` (optional bridge), `PATCHPILOT_MCP_ENABLED`, `SYFT_BIN`,
`CYCLONEDX_NPM_BIN`.
