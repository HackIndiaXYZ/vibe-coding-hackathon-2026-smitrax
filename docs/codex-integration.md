...
PatchPilot detects the Codex CLI with `CODEX_BIN` and `CODEX_ENABLED`.

Implemented:

- Codex availability health.
- Strict remediation prompt.
- `patchpilot-context.json` writer.
...
- **B.** A tiny no-repo prompt asking only for `{"ok":true,"message":"patchpilot codex check"}`.
